...
async def lifespan(app: FastAPI):
    await init_db()
    await poll_libre()       # Polling imediato no boot
    start_scheduler()
    yield


app = FastAPI(title="Glico API", version="1.0.0", lifespan=lifespan)

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],    # Restringir para o domínio do Vercel em produção
    allow_methods=["*"],
    allow_headers=["*"],
)
...
async def force_sync():
    """Força polling manual — útil para debug e para o botão Sync do frontend."""
    await poll_libre()
...
async def search_foods(q: str = Query(..., min_length=2)):
    """Busca fuzzy simples por nome de alimento no banco do manual."""
    q_lower = q.lower().strip()
    # Prioriza starts-with, depois contains
...
async def parse_meal(body: MealParseRequest):
    """
    Recebe descrição em texto livre e retorna itens estruturados
    prontos para confirmar e salvar via POST /meals.
    Ex: "comi um prato de arroz com feijão, uma laranja e pão francês"
    """
    if not body.description.strip():
        return []
    items = await parse_meal_text(body.description)
    return items
