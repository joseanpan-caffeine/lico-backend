...
# Versão 4.7.0 — não exige o header account-id que causava AccountIdMismatch
HEADERS = {
    "product": "llu.android",
    "version": "4.7.0",
...
class LibreClient:
    ...
    async def login(self, _redirect_count: int = 0) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            ...
            logger.info(f"Login OK | role: {jwt.get('role')} | country: {jwt.get('country')}")
            return True
    ...
    async def _get(self, path: str, _retried: bool = False) -> dict:
        ...
        # Sem account-id — versão 4.7.0 não exige
